...
def print_staging_summary(spec_labels, dependencies, stages):
    ...
    for stage in stages:
        ...
        for job in sorted(stage):
            ...
            tty.msg(
                "      [{1}] {0} -> {2}".format(
                    job, "x" if spec_labels[job]["needs_rebuild"] else " ", get_spec_string(s)
                )
            )
    ...
    for phase in phases:
        ...
        for stage_jobs in stages:
            ...
            for spec_label in stage_jobs:
                ...
                job_vars = {
                    "SPACK_ROOT_SPEC": format_root_spec(root_spec, main_phase, strip_compilers),
                ...
                if broken_specs_url:
                    broken_spec_path = url_util.join(broken_specs_url, release_spec_full_hash)
                ...
                if enable_cdash_reporting:
                    ...
                    if spec_label in dependencies:
                        related_builds = [
                            spec_labels[d]["spec"].name for d in dependencies[spec_label]
                        ]

                    job_vars["SPACK_CDASH_BUILD_NAME"] = cdash_build_name
                    job_vars["SPACK_RELATED_BUILDS_CDASH"] = ";".join(sorted(related_builds))
    ...
    for dep_pkg_name in dep_map:
        ...
        for url in cdashids_mirror_urls:
            try:
                ...
                tty.debug("Did not find cdashid for {0} on {1}".format(dep_pkg_name, url))
        else:
...
def write_cdashid_to_mirror(cdashid, spec, mirror_url):
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            msg = "Failed to write cdashid {0} to mirror {1}".format(cdashid, mirror_url)
            tty.warn(inst)
            tty.warn(msg)
...
def read_cdashid_from_mirror(spec, mirror_url):
    ...
    buildcache_name = bindist.tarball_name(spec, "")
    cdashid_file_name = "{0}.cdashid".format(buildcache_name)
    url = os.path.join(mirror_url, bindist.build_cache_relative_path(), cdashid_file_name)
...
def setup_spack_repro_version(repro_dir, checkout_commit, merge_commit=None):
    ...
    with fs.working_dir(repro_dir):
        clone_out = git("clone", spack_git_path, output=str, error=os.devnull, fail_on_error=False)
