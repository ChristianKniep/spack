...
class Spec(object):
    ...
    def dependencies_dict(self, deptype="all"):
        return dict((d.spec.name, d) for d in self._find_deps(self._dependencies, deptype))

    def dependents_dict(self, deptype="all"):
        return dict((d.parent.name, d) for d in self._find_deps(self._dependents, deptype))
    ...
    def _dup_deps(self, other, deptypes, caches):
        ...
        for dspec in other.traverse_edges(cover="edges", root=False):
            ...
            if dspec.parent.name not in new_specs:
                new_specs[dspec.parent.name] = dspec.parent.copy(deps=False, caches=caches)
...
class SpecParser(spack.parse.Parser):
    ...
    def spec_from_file(self):
        ...
        if not path.endswith(".yaml"):
            raise SpecFilenameError("Spec filename must end in .yaml: '{0}'".format(path))
