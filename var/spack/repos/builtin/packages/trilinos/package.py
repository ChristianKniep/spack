...
class Trilinos(CMakePackage, CudaPackage):
    ...
    variant("amesos2basker", default=False, description="Compile with Basker in Amesos2")
    ...
    def cmake_args(self):
        ...
        options.extend(
            [
                ...
                define_prefix_enable("EpetraExt_BUILD_EXPERIMENTAL", "epetraextexperimental"),
        ...
        if "+superlu-dist" in spec:
            options.extend(
                [
                    ...
                    define("SuperLUDist_INCLUDE_DIRS", spec["superlu-dist"].prefix.include),
                ]
            )
        ...
        if "+strumpack" in spec:
            options.extend(
                [
                    ...
                    define("STRUMPACK_LIBRARY_DIRS", spec["strumpack"].libs.directories[0]),
                    define("STRUMPACK_INCLUDE_DIRS", spec["strumpack"].headers.directories[0]),
                ]
            )
        ...
        options.append(
            define("Kokkos_ARCH_" + Kokkos.spack_micro_arch_map[spec.target.name].upper(), True)
        )
        ...
        if "+cuda" in spec:
            ...
            if "+cuda_rdc" in spec:
                options.append(define("Kokkos_ENABLE_CUDA_RELOCATABLE_DEVICE_CODE", True))
            for iArchCC in spec.variants["cuda_arch"].value:
                options.append(
                    define("Kokkos_ARCH_" + Kokkos.spack_cuda_arch_map[iArchCC].upper(), True)
                )
            if "+wrapper" in spec:
        ...
        if "+fortran" in spec and (
            spec.satisfies("%gcc") or spec.satisfies("%clang") or spec.satisfies("%apple-clang")
        ):
            options.append(define("Trilinos_ENABLE_Fortran", True))
            if "+mpi" in spec:
                libgfortran = os.path.dirname(
                    os.popen("%s --print-file-name libgfortran.a" % spec["mpi"].mpifc).read()
                )
                options.append(
                    define(
        ...
        options.append(
            self.define_from_variant(
                "Trilinos_ENABLE_EXPLICIT_INSTANTIATION", "explicit_template_instantiation"
            )
        )
