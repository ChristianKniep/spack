...
class FluxCore(AutotoolsPackage):
    ...
    def setup_run_environment(self, env):
        ...
        env.prepend_path("FLUX_PMI_LIBRARY_PATH", os.path.join(self.prefix.lib.flux, "libpmi.so"))
        ...
        if self.version < Version("0.12.0"):
            ...
            env.prepend_path(
                "FLUX_WRECK_LUA_PATTERN", os.path.join(self.prefix.etc.wreck, "lua.d", "*.lua")
            )
    ...
    def flag_handler(self, name, flags):
        if name == "cflags":
            ...
            if self.spec.satisfies("%gcc@10:") and self.spec.satisfies("@0.23.0:0.23.99"):
                if flags is None:
                    flags = []
